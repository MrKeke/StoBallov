    ...
    try { // пробуем сделать то что ниже чтобы в случае ошибки ничего не сломать
        ...
        const user = await prisma.user.create({ // создаем в базе нового пользователя
            data: {
                ...
                isAdmin: email === 'alexgraf0701@gmail.com', // проверяем регистрируется ли админ и отдаем базе данных
...
app.delete('/lesson/:id', async (req, res) => { // запрос на удаление урока
    try { // пробуем сделать то что ниже чтобы в случае ошибки ничего не сломать
        ...
        const deleted = await prisma.lesson.delete({ // удаляем из базы урок
...
app.post('/comment', async (req, res) => { // запрос на создание комментария
    try { // пробуем сделать то что ниже чтобы в случае ошибки ничего не сломать
        const token = req.headers.token // получаем токен пользователя
        const {description, lessonId} = req.body // получаем текст комментария и айди урока
        const user = jwt.verify(token, process.env.JWT_SECRET); // получаем пользователя из токена
        const comment = await prisma.comments.create({ // создаем в базе комментарий
            data: {
                description: description, // передаем тело комментария
                lessonId, // айди урока
                createrId: user.id, // айди создателя
            }
        })
    } catch (e) { // ловим ошибку
...
app.get('/comment/:id', async (req, res) => { // запрос на получение комментариев к уроку
    try { // пробуем сделать то что ниже чтобы в случае ошибки ничего не сломать
        const id = req.params.id; // получаем из запроса айди урока
        const comments = await prisma.comments.findMany({ // ищем комменатрии в базе
            where: {
                lessonId: Number(id)
            }
        })
        res.status(200).json({
            lessons: comments // возвращаем обратно комменты к уроку
        })
    } catch (e) { // ловим ошибку
        res.status(401).json({
            errors: e.message // возвращам ошибку
        })
    }
})
...
app.delete('/comment/:id', async (req, res) => { // запрос на удаление комментария
    try { // пробуем сделать то что ниже чтобы в случае ошибки ничего не сломать
        ...
        const deleted = await prisma.comments.delete({ // удаление комменатрия
